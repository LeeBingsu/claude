...
6. 공격 충전도가 요구치 이상 — 요구치는 매 타격마다 `autoAttackMinCharge`~`MaxCharge`
   (기본 0.85~0.95)에서 새로 뽑습니다

요구치는 연속값이지만 **실제 결과는 그렇지 않습니다.** 충전도는 틱 단위로만 오르므로
(검 기준 한 틱에 1/12.5 = 0.08씩), 범위 안의 여러 값이 결국 같은 틱에 발동합니다.
0.85~0.95는 실제로 **두 가지 결과**로만 떨어집니다.
...
| `autoAttackMinCharge` / `MaxCharge` | `0.85` / `0.95` | 매 타격마다 이 범위에서 요구 충전도를 새로 뽑습니다. 두 값을 같게 하면 고정값이 됩니다. |
